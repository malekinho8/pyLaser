...
def run(laser_azimuth, laser_polar, mirror_roll, mirror_pitch, mirror_yaw):
    ...
    visualize_beam_and_surface(incident_vector, reflected_vector)
...
def visualize_beam_and_surface(incident_vector, reflected_vector):
    ...
    fig.add_trace(go.Scatter3d(
        x=[0, incident_vector[0]], # 0 indicates the starting value
        y=[0, incident_vector[1]],
        z=[0, incident_vector[2]],
        name='Incident Beam',
        mode='lines',
        line=dict(
            color='blue',
    ...
    # set the axes labels and title with LaTeX
    fig.update_layout(
        scene=dict(
            xaxis_title='$X$',
            yaxis_title='$Y$',
        ...
        title_text=r'$\text{Incident and Reflected Beams}$',
        autosize=True, # this makes the plot fit the available space
    )
